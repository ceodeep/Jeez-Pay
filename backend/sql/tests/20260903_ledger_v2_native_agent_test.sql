...
\echo 'ROLLBACK-ONLY: temporarily promotes one existing SSP user to agent, executes tiny cash-in/cash-out through Phase 4.4B, verifies atomic agent_operations + native Ledger posting/idempotency/no mirror double-post, then rolls everything back.'
...
BEGIN
  ...
  IF v_agent_id IS NULL THEN
    RAISE EXCEPTION 'PHASE44_TEST_NO_AGENT_CANDIDATE';
  END IF;
  ...
  -- No permanent agent exists in production yet. Temporarily promote the
  -- candidate only inside this rollback-only transaction.
  UPDATE public.users
  SET role = 'agent'
  WHERE id = v_agent_id;
