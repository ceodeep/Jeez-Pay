...
CREATE TABLE IF NOT EXISTS public.ledger_accounts_v2 (
  ...
  CONSTRAINT ledger_accounts_v2_account_key_not_blank CHECK (btrim(account_key) <> ''),
  CONSTRAINT ledger_accounts_v2_account_key_length CHECK (length(account_key) <= 200),
  CONSTRAINT ledger_accounts_v2_account_type_not_blank CHECK (btrim(account_type) <> ''),
  CONSTRAINT ledger_accounts_v2_account_type_length CHECK (length(account_type) <= 80),
  CONSTRAINT ledger_accounts_v2_owner_type_not_blank CHECK (btrim(owner_type) <> ''),
  CONSTRAINT ledger_accounts_v2_owner_type_length CHECK (length(owner_type) <= 80),
  CONSTRAINT ledger_accounts_v2_currency_format CHECK (currency ~ '^[A-Z0-9]{3,10}$'),
  CONSTRAINT ledger_accounts_v2_status_check CHECK (status IN ('active', 'frozen', 'closed')),
  CONSTRAINT ledger_accounts_v2_metadata_object CHECK (jsonb_typeof(metadata) = 'object')
);
...
CREATE TABLE IF NOT EXISTS public.ledger_account_balances_v2 (
  account_id uuid PRIMARY KEY REFERENCES public.ledger_accounts_v2(id) ON DELETE RESTRICT,
  balance numeric(38, 12) NOT NULL DEFAULT 0,
  version bigint NOT NULL DEFAULT 0,
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ledger_account_balances_v2_version_nonnegative CHECK (version >= 0)
);
...
CREATE TABLE IF NOT EXISTS public.ledger_journals_v2 (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ledger_journals_v2_source_type_not_blank CHECK (btrim(source_type) <> ''),
  CONSTRAINT ledger_journals_v2_source_type_length CHECK (length(source_type) <= 80),
  CONSTRAINT ledger_journals_v2_idempotency_key_not_blank CHECK (btrim(idempotency_key) <> ''),
  CONSTRAINT ledger_journals_v2_idempotency_key_length CHECK (length(idempotency_key) <= 200),
  CONSTRAINT ledger_journals_v2_request_hash_format CHECK (request_hash ~ '^[0-9a-f]{64}$'),
  CONSTRAINT ledger_journals_v2_metadata_object CHECK (jsonb_typeof(metadata) = 'object'),
  CONSTRAINT ledger_journals_v2_source_idempotency_uidx UNIQUE (source_type, idempotency_key)
);
...
CREATE TABLE IF NOT EXISTS public.ledger_entries_v2 (
  ...
  account_id uuid NOT NULL REFERENCES public.ledger_accounts_v2(id) ON DELETE RESTRICT,
  currency text NOT NULL,
  amount_delta numeric(38, 12) NOT NULL,
  description text,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ledger_entries_v2_currency_format CHECK (currency ~ '^[A-Z0-9]{3,10}$'),
  CONSTRAINT ledger_entries_v2_nonzero_amount CHECK (amount_delta <> 0),
  CONSTRAINT ledger_entries_v2_metadata_object CHECK (jsonb_typeof(metadata) = 'object'),
  CONSTRAINT ledger_entries_v2_one_line_per_account UNIQUE (journal_id, account_id)
);
...
DROP TRIGGER IF EXISTS ledger_accounts_v2_initialize_balance ON public.ledger_accounts_v2;
CREATE TRIGGER ledger_accounts_v2_initialize_balance
AFTER INSERT ON public.ledger_accounts_v2
FOR EACH ROW EXECUTE FUNCTION public.initialize_ledger_account_balance_v2();

INSERT INTO public.ledger_account_balances_v2 (account_id, balance, version)
SELECT id, 0, 0 FROM public.ledger_accounts_v2
ON CONFLICT (account_id) DO NOTHING;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'LEDGER_ACCOUNT_IDENTITY_IMMUTABLE' USING ERRCODE = 'P0001';
  END IF;
  NEW.updated_at := now();
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS ledger_accounts_v2_guard_identity ON public.ledger_accounts_v2;
CREATE TRIGGER ledger_accounts_v2_guard_identity
BEFORE UPDATE ON public.ledger_accounts_v2
FOR EACH ROW EXECUTE FUNCTION public.guard_ledger_account_identity_v2();

CREATE OR REPLACE FUNCTION public.reject_ledger_v2_immutable_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'LEDGER_V2_IMMUTABLE_RECORD' USING ERRCODE = 'P0001';
END;
$$;

DROP TRIGGER IF EXISTS ledger_journals_v2_immutable ON public.ledger_journals_v2;
CREATE TRIGGER ledger_journals_v2_immutable
BEFORE UPDATE OR DELETE ON public.ledger_journals_v2
FOR EACH ROW EXECUTE FUNCTION public.reject_ledger_v2_immutable_mutation();

DROP TRIGGER IF EXISTS ledger_entries_v2_immutable ON public.ledger_entries_v2;
...
BEGIN
  IF current_setting('jeezpay.ledger_posting_v2', true) IS DISTINCT FROM 'on' THEN
    RAISE EXCEPTION 'LEDGER_BALANCE_DIRECT_MUTATION_FORBIDDEN' USING ERRCODE = 'P0001';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS ledger_account_balances_v2_guard_update ON public.ledger_account_balances_v2;
CREATE TRIGGER ledger_account_balances_v2_guard_update
BEFORE UPDATE OR DELETE ON public.ledger_account_balances_v2
FOR EACH ROW EXECUTE FUNCTION public.guard_ledger_balance_mutation_v2();
...
BEGIN
  ...
  IF FOUND THEN
    ...
    THEN
      RAISE EXCEPTION 'LEDGER_ACCOUNT_KEY_CONFLICT' USING ERRCODE = 'P0001';
    END IF;
    RETURN v_existing.id;
  END IF;

  INSERT INTO public.ledger_accounts_v2 (
    account_key, account_type, owner_type, owner_ref, currency, allow_negative, metadata
  ) VALUES (
    v_account_key, v_account_type, v_owner_type, v_owner_ref,
    v_currency, COALESCE(p_allow_negative, false), v_metadata
  )
...
BEGIN
  ...
  SELECT count(*) INTO v_bad_count
  FROM (
    SELECT upper(btrim(e->>'currency')) AS currency,
  ...
  WHERE source_type = v_source_type AND idempotency_key = v_idempotency_key;

  IF FOUND THEN
    IF v_existing_hash <> v_request_hash THEN
      RAISE EXCEPTION 'LEDGER_IDEMPOTENCY_CONFLICT' USING ERRCODE = 'P0001';
    END IF;
    RETURN jsonb_build_object('ok', true, 'journalId', v_existing_id, 'idempotentReplay', true);
  END IF;
  ...
  WHERE a.id = ANY(v_account_ids) AND a.status <> 'active';
  IF v_bad_count > 0 THEN
  ...
  SELECT count(*) INTO v_bad_count
  FROM jsonb_array_elements(p_entries) AS e
  JOIN public.ledger_accounts_v2 AS a ON a.id = (e->>'accountId')::uuid
  ...
  SELECT count(*) INTO v_bad_count
  FROM unnest(v_account_ids) AS requested(account_id)
  LEFT JOIN public.ledger_account_balances_v2 AS b ON b.account_id = requested.account_id
  ...
  FROM jsonb_array_elements(p_entries) AS e
  JOIN public.ledger_accounts_v2 AS a ON a.id = (e->>'accountId')::uuid
  JOIN public.ledger_account_balances_v2 AS b ON b.account_id = a.id
  ...
  BEGIN
    INSERT INTO public.ledger_journals_v2 (
      source_type, source_ref, idempotency_key, request_hash, description, metadata
    ) VALUES (
      v_source_type, v_source_ref, v_idempotency_key, v_request_hash, p_description, v_metadata
  ...
  INSERT INTO public.ledger_entries_v2 (
    journal_id, account_id, currency, amount_delta, description, metadata
  )
  SELECT v_journal_id,
         (e->>'accountId')::uuid,
  ...
  SET balance = b.balance + delta.amount_delta,
      version = b.version + 1,
      updated_at = now()
  FROM (
    SELECT (e->>'accountId')::uuid AS account_id,
  ...
  RETURN jsonb_build_object('ok', true, 'journalId', v_journal_id, 'idempotentReplay', false);
END;
$$;

CREATE OR REPLACE VIEW public.ledger_v2_unbalanced_journals
WITH (security_invoker = true) AS
SELECT e.journal_id, e.currency, sum(e.amount_delta) AS net_delta
FROM public.ledger_entries_v2 AS e
GROUP BY e.journal_id, e.currency
HAVING sum(e.amount_delta) <> 0;

CREATE OR REPLACE VIEW public.ledger_v2_balance_reconciliation
WITH (security_invoker = true) AS
SELECT
  a.id AS account_id,
  a.account_key,
...
FROM public.ledger_accounts_v2 AS a
JOIN public.ledger_account_balances_v2 AS b ON b.account_id = a.id
LEFT JOIN public.ledger_entries_v2 AS e ON e.account_id = a.id
GROUP BY a.id, a.account_key, a.account_type, a.owner_type, a.owner_ref,
         a.currency, b.balance, b.version, b.updated_at;
...
REVOKE ALL ON TABLE
  ...
  public.ledger_v2_unbalanced_journals,
  public.ledger_v2_balance_reconciliation
FROM PUBLIC;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    EXECUTE 'REVOKE ALL ON TABLE public.ledger_accounts_v2, public.ledger_account_balances_v2, public.ledger_journals_v2, public.ledger_entries_v2, public.ledger_v2_unbalanced_journals, public.ledger_v2_balance_reconciliation FROM anon';
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.ensure_ledger_account_v2(text, text, text, text, text, boolean, jsonb) FROM anon';
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.post_ledger_journal_v2(text, text, text, text, jsonb, jsonb) FROM anon';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    EXECUTE 'REVOKE ALL ON TABLE public.ledger_accounts_v2, public.ledger_account_balances_v2, public.ledger_journals_v2, public.ledger_entries_v2, public.ledger_v2_unbalanced_journals, public.ledger_v2_balance_reconciliation FROM authenticated';
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.ensure_ledger_account_v2(text, text, text, text, text, boolean, jsonb) FROM authenticated';
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.post_ledger_journal_v2(text, text, text, text, jsonb, jsonb) FROM authenticated';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    EXECUTE 'REVOKE ALL ON TABLE public.ledger_accounts_v2, public.ledger_account_balances_v2, public.ledger_journals_v2, public.ledger_entries_v2, public.ledger_v2_unbalanced_journals, public.ledger_v2_balance_reconciliation FROM service_role';
